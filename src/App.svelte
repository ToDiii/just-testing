...
<main class="bg-gray-100 min-h-screen font-sans">
  <header class="bg-white shadow">
    <div class="container mx-auto px-4 py-6">
      ...
      <nav class="mt-4">
        <button
          class="px-4 py-2 rounded-md mr-2 {currentView === 'map' ? 'bg-blue-500 text-white' : 'bg-gray-200'}"
          on:click={() => currentView = 'map'}>
          Map
        </button>
        <button
          class="px-4 py-2 rounded-md mr-2 {currentView === 'results' ? 'bg-blue-500 text-white' : 'bg-gray-200'}"
          on:click={() => currentView = 'results'}>
          Results
        </button>
        <button
          class="px-4 py-2 rounded-md {currentView === 'targets' ? 'bg-blue-500 text-white' : 'bg-gray-200'}"
  ...
  <div class="fixed bottom-2 right-2 bg-gray-800 text-white text-xs p-2 rounded shadow-lg z-50">
